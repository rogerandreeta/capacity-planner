...
function CapacityPlanner() {
  ...
  const [people, setPeople] = useState([
    { id: 1, name: 'Sarah Chen', role: 'UX Designer', hoursPerWeek: 38 },
    { id: 2, name: 'Marcus Rodriguez', role: 'Frontend Developer', hoursPerWeek: 38 },
  ]);
  ...
  const addPerson = () => {
    ...
    setPeople([...people, { id: newId, name: '', role: '', hoursPerWeek: 38 }]);
  };
                    ...
                    <div key={resource.personId} style={{ display: 'flex', alignItems: 'center', padding: '8px 0 8px 0', borderBottom: isLast ? 'none' : '1px solid #e2e8f0' }}>
                      ...
                      <div style={{ width: LABEL_W, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingLeft: 10, paddingRight: 6 }}>
                        <div style={{ maxWidth: 90, overflow: 'hidden' }}>
                          <span className="text-xs font-semibold text-slate-700 truncate block">{person.name}</span>
                          {person.role && <span className="text-[10px] text-indigo-500 truncate block">{person.role}</span>}
                        </div>
                        <button
          ...
          {people.map((person) => (
            <div key={person.id} className="flex items-center mb-2">
              <div className="w-48 flex-shrink-0 pr-4">
                ...
                {person.role && <div className="text-xs text-indigo-500 truncate">{person.role}</div>}
